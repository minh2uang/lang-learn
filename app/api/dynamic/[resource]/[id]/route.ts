...
export const GET = async (
  ...
  return Response.json(
    await DbService().getCollection(resource).findOne({
      _id,
    })
  );
};
...
export const DELETE = async (
  ...
  const _id: string = params.id;
  return Response.json(
    await DbService().getCollection(resource).findOneAndDelete({
      _id,
    })
  );
};
...
export const PATCH = async (
  ...
  return Response.json(
    await DbService().getCollection(resource).findOneAndUpdate(
      { _id },
      {
        $set: rest,
      }
